...
//script adapted from https://nerdhut.de/2020/05/09/unity-arcball-camera-spherical-coordinates/
...
namespace tetris3D.controller
{
    ...
    public class ArcballScript : MonoBehaviour
    {
        ...
        void Start ()
        {
            this.transform.position = new Vector3 (radius, 0.0f, 0.0f) + target;
            this.transform.LookAt (target);
            sc = getSphericalCoordinates (this.transform.position);
            if (coolColors)
            {
                _camera = Camera.main;
                StartCoroutine(MakeCoolColors());
            }
        }

        private IEnumerator MakeCoolColors()
        {
            while (true)
            {
                float t = 0;
                Color currColor = _camera.backgroundColor;
                Color nextColor = new Color(Random.Range(0.157f, 1f), Random.Range(0.157f, 1f), Random.Range(0.157f, 1f));
                while (t <= 1)
                {
                    _camera.backgroundColor = Color.Lerp(currColor, nextColor, t);
                    t += Time.deltaTime / 5;
                    yield return null;
                }
